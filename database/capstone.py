#Installing the Requied modules
...
df = pd.DataFrame.from_records(raw_datasets)

#Pre-processing the Data Obtained
data.drop_duplicates(subset=['document'],inplace=True)  #dropping duplicates
...
length_df = pd.DataFrame({'document':text_word_count, 'summary':summary_word_count})
length_df.hist(bins = 30)
plt.show()

max_length = data.apply(lambda x: x.str.len().max())
max_length
